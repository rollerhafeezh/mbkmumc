...
<body>
	...
	<table border="0" width="100%" class="valign-top">
		...
		<tr>
			<td valign="top">Lokasi Penelitian</td>
			<td  valign="top">:</td>
	...
	<table border="1" width="100%" cellpadding="5" cellspacing="0">
		...
		<tr bgcolor="silver">
			<?php $komponen_nilai = $this->Aktivitas_model->komponen_nilai([ 'aktif' => 1 ])->result() ?>
		...
		<tr>
			...
			<td align="center" style="<?= $nilai == 0 ? 'background: white; color: white;' : '' ?>"><?= ($i > 0 ? ($nilai / $i) : 0); $nilai_pembimbing += ($i > 0 ? ($nilai / $i) : 0); ($nilai != 0 ? $penilai++ : $penilai) ?></td>
		</tr>
		<?php endforeach; ?>
		...
		<tr>
			...
			<td align="center" style="<?= $nilai == 0 ? 'background: white; color: white;' : '' ?>"><?= ($i > 0 ? ($nilai / $i) : 0); $nilai_penguji += ($i > 0 ? ($nilai / $i) : 0); ($nilai != 0 ? $penilai++ : $penilai) ?></td>
		</tr>
		<?php endforeach; ?>
		...
		<tr>
			...
			<?php $nilai_ketua_sidang = 0; foreach($komponen_nilai as $komponen_nilai_): ?>
			<?php $nilai = $this->Aktivitas_model->nilai([ 'id_penjadwalan' => $penjadwalan->id_penjadwalan, 'id_komponen_nilai' => $komponen_nilai_->id_komponen_nilai, 'jenis_nilai' => 3, 'nidn' => $penjadwalan->nidn ])->row() ?>
			<td align="center" style="<?= $nilai == 0 ? 'background: white; color: white;' : '' ?>"><?= $nilai_ketua_sidang = ($nilai ? $nilai->nilai : 0) ?></td>
			<?php endforeach; ?>

			<td align="center" style="<?= $nilai_ketua_sidang == 0 ? 'background: white; color: white;' : '' ?>"><?= $nilai_ketua_sidang; $penilai += ($nilai_ketua_sidang == 0 ? 0 : 1) ?></td>
		</tr>
